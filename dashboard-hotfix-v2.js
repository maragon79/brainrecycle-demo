(function(){
  ...
  function patchTrace(){
    ...
    if(!note){note=document.createElement('div');note.id='br-trh-40d-note';note.style.cssText='background:#e7f0dd;border:1px solid #d3d9d1;border-radius:10px;padding:12px 14px;font-size:12.5px;line-height:1.5;color:#48564f;max-width:100%;margin-top:8px';tabs.insertAdjacentElement('afterend',note);}
    note.innerHTML=EN?'<strong style="color:#1b2721">Demonstration time balance · Reference HRT: 40 days.</strong> Feeding recorded on <strong>08 Jul 2026 · 06:18</strong>; anaerobic digestion starts at <strong>06:25</strong>. The temporally associated digestate output is shown on <strong>17 Aug 2026 · 06:25</strong>. Demo balance: <strong>27.6 t equivalent digestate</strong>; <strong>9.4 t</strong> are allocated to plot ZA-02 and <strong>18.2 t</strong> remain in storage. In a continuous digester this is a time-window/mass-balance association, not exact physical tracking of an individual batch.':'<strong style="color:#1b2721">Ejemplo de balance temporal · TRH de referencia: 40 días.</strong> Alimentación registrada el <strong>08 jul 2026 · 06:18</strong>; la digestión anaerobia comienza a las <strong>06:25</strong>. La salida de digestato temporalmente asociada se representa el <strong>17 ago 2026 · 06:25</strong>. Balance demo: <strong>27,6 t equivalentes de digestato</strong>; <strong>9,4 t</strong> se asignan a la parcela ZA-02 y <strong>18,2 t</strong> permanecen en almacenamiento. En un digestor continuo es una asociación por ventana temporal/balance de masas, no una trazabilidad física exacta de un lote individual.';

    var buttons=document.querySelectorAll('button');
    var moments={
      'Alimentación':'08 jul · 06:18','Feeding':'08 Jul · 06:18',
      'Digestión anaerobia':'08 jul · 06:25 · TRH 40 días','Anaerobic digestion':'08 Jul · 06:25 · HRT 40 days',
      'Digestato':'17 ago · 06:25','Digestate':'17 Aug · 06:25',
      'Vías de valorización':'17 ago · 07:18','Valorization routes':'17 Aug · 07:18',
      'Campo / R10':'18 ago · 09:15','Field / R10':'18 Aug · 09:15',
      'Administración':'18 ago · 09:30','Administration':'18 Aug · 09:30'
    };
    for(var i=0;i<buttons.length;i++){
      var spans=buttons[i].querySelectorAll('span');if(spans.length<2)continue;var lab=text(spans[0]);if(moments[lab])spans[1].textContent=moments[lab];
    }
    ...
    var exactDates={
      ...
      'Stock asociado':EN?'27.6 t equivalent digestate · temporal balance':'27,6 t equivalentes de digestato · balance temporal'
    };
    var w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT),tn;
    while((tn=w.nextNode())){var c=(tn.nodeValue||'').trim();if(exactDates[c]){var l=(tn.nodeValue.match(/^\s*/)||[''])[0],r=(tn.nodeValue.match(/\s*$/)||[''])[0];tn.nodeValue=l+exactDates[c]+r;}}

    var scroller=null,divs=document.querySelectorAll('div');
    for(var d=0;d<divs.length;d++){var s=text(divs[d]);if((s.indexOf('Origen')>=0||s.indexOf('Origin')>=0)&&(s.indexOf('Digestión anaerobia')>=0||s.indexOf('Anaerobic digestion')>=0)&&divs[d].scrollWidth>divs[d].clientWidth+20){scroller=divs[d];break;}}
    if(scroller){
      scroller.style.overflowX='auto';scroller.style.webkitOverflowScrolling='touch';scroller.style.paddingBottom='12px';
      var hint=document.getElementById('br-trace-scroll-hint');
      if(!hint){hint=document.createElement('div');hint.id='br-trace-scroll-hint';hint.style.cssText='font-size:12px;font-weight:700;color:#3f685b;background:#eef5e7;border:1px solid #d3d9d1;border-radius:999px;padding:7px 12px;width:fit-content;max-width:100%;margin:0 0 8px 0';scroller.insertAdjacentElement('beforebegin',hint);}
      hint.textContent=EN?'Swipe to the right to view the full traceability →':'Desliza hacia la derecha para ver toda la trazabilidad →';
      var bar=document.getElementById('br-trace-scrollbar');
      if(!bar){bar=document.createElement('div');bar.id='br-trace-scrollbar';bar.style.cssText='height:8px;background:#dfe5dd;border-radius:999px;position:relative;margin-top:6px;overflow:hidden';bar.innerHTML='<div id="br-trace-scrollthumb" style="position:absolute;left:0;top:0;height:8px;background:#789083;border-radius:999px;min-width:44px"></div>';scroller.insertAdjacentElement('afterend',bar);}
