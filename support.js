...
(function(){
  ...
  function fixHomeOriginalLang(){
    if(!isHome() || !window.matchMedia('(max-width:760px)').matches) return false;
    var b=document.querySelector('header .header-actions button');
    if(!b) return false;
    if(!b.dataset.brLangState){
      b.dataset.brLangState='en';
      b.addEventListener('click',function(){
        var next=b.dataset.brLangState==='en'?'es':'en';
        b.dataset.brLangState=next;
        setTimeout(function(){
          b.textContent=next==='en'?'🇬🇧 EN':'🇪🇸 ES';
          var fixed=document.querySelector('#br-mobile-fixed-nav .br-fixed-lang');
          if(fixed) fixed.textContent=b.textContent;
        },0);
      });
    }
    if(/\{\{|langOtherFlag|langOther/.test(b.textContent||'')){
      b.textContent=b.dataset.brLangState==='es'?'🇪🇸 ES':'🇬🇧 EN';
    }
    return true;
  }

  function installMobileHeader(){
    if(!isHome() || !window.matchMedia('(max-width:760px)').matches) return true;
    if(document.getElementById('br-mobile-fixed-nav')) return true;
    ...
    if(!originalBar) return false;
    var logo=originalBar.querySelector('img[alt="brainrecycle®"],img[src*="logo-light"]');
    if(!document.getElementById('br-mobile-fixed-nav-style')){
  ...
  function enhance(){
    var homeReady=fixHomeOriginalLang() && installMobileHeader();
    var dashReady=emphasizeDashboardMenu();
    return homeReady || dashReady;
  }

  /* Bounded retries only. Avoid a document-wide MutationObserver: Jotform's AI widget
     performs many DOM mutations while booting, and observing every mutation delays it. */
  function startEnhanceRetries(){
    var tries=0,maxTries=30;
    function tick(){
      tries++;
      enhance();
      if(tries<maxTries && !document.getElementById('br-mobile-fixed-nav') && !document.querySelector('.dash-mobile-menu-btn[data-br-emphasized="1"]')){
        setTimeout(tick,100);
      }
    }
    tick();
  ...
  s.async=false;
  s.onload=function(){
    if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',startEnhanceRetries,{once:true});
    else startEnhanceRetries();
  };
