...
/* BrainRecycle V5.9 — responsive integral zona privada + corrección TRH digestión */
(() => {
  const STYLE_ID = 'br-v59-responsive-private-area';
  const css = `
@media (max-width:760px){
  html,body{max-width:100%!important;overflow-x:hidden!important}
  html body x-dc .dash-main,
  html body x-dc .dash-main > *,
  html body x-dc .dash-main section,
  html body x-dc .dash-main article{min-width:0!important;max-width:100%!important}

  /* Todas las rejillas de escritorio pasan a una columna en móvil. */
  html body x-dc .dash-main [style*="grid-template-columns"]{
  ...
  /* Tarjetas y bloques: nunca pueden salirse del viewport. */
  html body x-dc .dash-main [style*="border-radius"],
  html body x-dc .dash-main [style*="background:var(--surface-raised)"],
  html body x-dc .dash-main [style*="background:#fff"],
  html body x-dc .dash-main .card{
    max-width:100%!important;
    min-width:0!important;
    overflow-wrap:anywhere!important;
  }
  html body x-dc .dash-main p,
  html body x-dc .dash-main h1,
  html body x-dc .dash-main h2,
  html body x-dc .dash-main h3,
  html body x-dc .dash-main h4,
  html body x-dc .dash-main span,
  html body x-dc .dash-main strong,
  html body x-dc .dash-main a{
    max-width:100%!important;
    overflow-wrap:anywhere!important;
    word-break:normal!important;
  }

  /* Padding proporcionado en zona privada. */
  ...
  html body x-dc .dash-main [style*="padding:20px"]{padding:15px 14px!important}

  /* Tipografía de títulos de escritorio. */
  html body x-dc .dash-main [style*="font-size:36px"]{font-size:28px!important;line-height:1.08!important}
  html body x-dc .dash-main [style*="font-size:34px"]{font-size:27px!important;line-height:1.1!important}
  html body x-dc .dash-main [style*="font-size:32px"]{font-size:26px!important;line-height:1.1!important}
  ...
  /* Tablas: conservar información, con scroll horizontal controlado si no cabe. */
  html body x-dc .dash-main table{
    display:block!important;
    width:100%!important;
    max-width:100%!important;
  ...
  html body x-dc .dash-main table thead,
  html body x-dc .dash-main table tbody,
  html body x-dc .dash-main table tfoot{
    min-width:620px!important;
  }
  html body x-dc .dash-main table th,
  html body x-dc .dash-main table td{
    white-space:normal!important;
    overflow-wrap:anywhere!important;
  }

  /* Elementos que necesariamente son horizontales (líneas de vida, rutas, procesos): scroll visible y usable. */
  html body x-dc .dash-main .br-mobile-horizontal,
  html body x-dc .dash-main [data-br-horizontal="true"]{
    width:100%!important;
    max-width:100%!important;
    overflow-x:auto!important;
  ...
  /* Flex complejos no deben quedar cortados. */
  html body x-dc .dash-main .br-mobile-stack{
  ...
  /* Formularios/botones largos. */
  html body x-dc .dash-main input,
  html body x-dc .dash-main select,
  html body x-dc .dash-main textarea,
  html body x-dc .dash-main button{
    max-width:100%!important;
  }

  /* Medios y gráficos. */
  html body x-dc .dash-main img,
  html body x-dc .dash-main svg,
  html body x-dc .dash-main canvas,
...
`;
  ...
  function fixConceptualFlow(root=document){
    /* La entrada individual deja de representarse como lote físico tras la mezcla en reactor continuo. */
    root.querySelectorAll('*').forEach(el=>{
      const t=txt(el);
      if(!t || el.children.length) return;
      if(t==='Digestato') el.textContent='Digestato generado';
      else if(t==='07:10') el.textContent='≈ Día +40';
      else if(t==='Almacenamiento digestato') el.textContent='Generación / almacenamiento de digestato';
      else if(t==='08 jul 2026 · 07:10') el.textContent='≈ 17 ago 2026 · tras ~40 días';
      else if(t==='Digestato + caracterización') el.textContent='Digestato resultante + caracterización';
      else if(t==='Producción · cantidad · fracción · stock · almacenamiento · caracterización · analíticas · genealogía de materias de entrada · proceso y evidencias.'){
        el.textContent='Producción · cantidad · fracción · stock · almacenamiento · caracterización · analíticas · genealogía de materias de entrada · proceso y evidencias. Tras la mezcla en un digestor continuo no se atribuye físicamente el digestato a una cisterna concreta: se conservan ventanas temporales, tiempos de residencia y contribuciones estimadas.';
      }
      else if(t==='Materias de entrada + proceso + analíticas + trazabilidad forman la cadena de evidencia para evaluar destinos posteriores.'){
        el.textContent='Tras el tiempo de retención hidráulico del proceso (≈40 días en esta demostración), el purín forma parte de una corriente de digestato generada junto con el resto de materias alimentadas. La cadena de evidencia permite evaluar después vías como aplicación agronómica/R10, valorización o preparación para fertilizante UE cuando proceda.';
      }
    });

    /* Añadir una nota técnica inequívoca junto a la etapa de digestato si todavía no existe. */
    const candidates=[...root.querySelectorAll('*')].filter(e=>txt(e)==='Digestato generado');
    candidates.forEach(label=>{
      const box=label.parentElement?.parentElement || label.parentElement;
      if(!box || box.querySelector('.br-trh-note')) return;
      const note=document.createElement('div');
      note.className='br-trh-note';
      note.textContent='TRH de demostración ≈ 40 días · la cisterna ya forma parte de la mezcla del digestor';
      note.style.cssText='font-size:11px;line-height:1.35;color:var(--text-secondary);margin-top:5px;max-width:190px';
      box.appendChild(note);
    });
  }

  function classifyResponsive(root=document){
    if(window.innerWidth>760) return;
    const main=root.querySelector('.dash-main')||root;

    /* Detectar líneas de proceso horizontales por semántica visible. */
    main.querySelectorAll('div,section').forEach(el=>{
      const t=txt(el);
      const childCount=el.children.length;
      if(childCount>=4 && childCount<=14 && /Alimentación|Digestión anaerobia|Circuito regulatorio|Administración|Campo \/ R10|Vías de valorización/.test(t)){
        el.classList.add('br-mobile-horizontal');
      }
    });

    /* Flex de 2–4 tarjetas grandes: apilar en móvil. */
    main.querySelectorAll('div,section').forEach(el=>{
      const cs=getComputedStyle(el);
      if(cs.display!=='flex' || el.classList.contains('br-mobile-horizontal')) return;
      const kids=[...el.children].filter(c=>c.nodeType===1);
      if(kids.length<2 || kids.length>4) return;
      const large=kids.filter(k=>k.getBoundingClientRect().width>180).length;
      if(large>=2) el.classList.add('br-mobile-stack');
    });

    /* Parents de tabla pueden scrollear, evitando que un overflow hidden la corte. */
    main.querySelectorAll('table').forEach(table=>{
      let p=table.parentElement;
      for(let i=0;i<2 && p;i++,p=p.parentElement){
        p.style.setProperty('max-width','100%','important');
        p.style.setProperty('overflow-x','auto','important');
  ...
  window.addEventListener('resize',()=>{ if(window.innerWidth<=760) classifyResponsive(); },{passive:true});
  const mo=new MutationObserver(()=>{fixConceptualFlow(); if(window.innerWidth<=760) classifyResponsive();});
