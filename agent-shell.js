...
(function(){
  ...
  function init(){
    ...
    style.textContent=`
      #br-agent-launcher{position:fixed;right:18px;bottom:18px;z-index:9200;width:68px;height:68px;border:3px solid #fff;border-radius:50%;background:#2c483d;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 10px 28px rgba(0,0,0,.28);cursor:pointer;-webkit-tap-highlight-color:transparent}
      #br-agent-launcher svg{width:31px;height:31px;display:block}
      #br-agent-backdrop{display:none;position:fixed;left:0;right:0;bottom:0;top:72px;z-index:8800;background:rgba(8,22,16,.52);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px)}
      #br-agent-panel{display:none;position:fixed;right:14px;bottom:14px;z-index:9000;width:min(430px,calc(100vw - 28px));height:min(68dvh,610px);max-height:calc(100dvh - 112px);min-height:430px;background:#f6faf4;border-radius:20px;overflow:hidden;box-shadow:0 22px 60px rgba(0,0,0,.34);border:1px solid rgba(44,72,61,.16)}
